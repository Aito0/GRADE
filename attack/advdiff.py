...
class AdvDiffAttack(object):
    def __init__(self, schedule="linear", vic_model=None, **kwargs):
    ...
    def load(self):
        ...
        self.model = load_model_from_config(config, "models/ldm/cin256-v2/model.ckpt")
        self.ddpm_num_timesteps = self.model.num_timesteps
        
        self.vic_model = resnet50(weights=ResNet50_Weights.IMAGENET1K_V2).to(self.model.device)
        self.vic_model.eval()
        
    def generate_examples(self, classes=None, n_samples_per_class=6, ddim_steps=200, 
                           ddim_eta=0.0, scale=3.0, K=5, s=1.0, a=0.5):
        ...
        with torch.no_grad():
            with self.model.ema_scope():
                ...
                for class_label in classes:
                    print(f"rendering {n_samples_per_class} examples of class '{class_label}' "
                          f"in {ddim_steps} steps and using s={scale:.2f}.")
                    
                    xc = torch.tensor(n_samples_per_class * [class_label])
                    c = self.model.get_learned_conditioning(
                        {self.model.cond_stage_key: xc.to(self.model.device)}
                    )
